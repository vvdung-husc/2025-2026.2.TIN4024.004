...
Traffic_Blink::Traffic_Blink()
{
    ...
    _secondCount = 0;
    // Khởi tạo mặc định tránh lỗi chia cho 0
    _waitTime[0] = 7000; _waitTime[1] = 3000; _waitTime[2] = 5000;
}
...
void Traffic_Blink::blink(bool showLogger)
{
    ...
    {
        // Hiệu ứng nhấp nháy đèn giao thông
        ledStatus = !ledStatus;
...
void SENSOR_LDR::setup(int pin)
{
    _pin = pin;
    pinMode(_pin, INPUT);
}

int SENSOR_LDR::getLux()
{
    return analogRead(_pin); // ESP32: 0 - 4095
}

bool SENSOR_LDR::isDark(int limit)
{
    // Giả sử mạch phân áp: Tối -> Điện trở LDR tăng -> Áp vào Pin thay đổi.
    // Logic phổ biến: Tối = Giá trị cao.
    return (getLux() > limit);
}
