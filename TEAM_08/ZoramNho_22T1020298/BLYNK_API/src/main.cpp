#define BLYNK_TEMPLATE_ID "TMPL6c3rRbKYd"
#define BLYNK_TEMPLATE_NAME "BLYNK API"
#define BLYNK_AUTH_TOKEN "QrRIkTXYaoIsyx9vW9J_JZSi9AUBDVQF"
...
const char OPENWEATHER_API_KEY[] = "022cb210c67d678232b0013d1384fbc9";
...
// Hàm gửi Uptime lên V0 (thoiGianHoatDong)
void sendUptime() {
...
void fetchGeoLocation() {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpCode > 0) {
      ...
      Blynk.virtualWrite(V1, myIP);    // V1: DiachiIP
      Blynk.virtualWrite(V2, mapLink); // V2: GGMap
    } else {
  ...
  if (WiFi.status() == WL_CONNECTED && myLat.length() > 0 && myLon.length() > 0) {
    ...
    if (httpCode > 0) {
      ...
      if (!error) {
        ...
        Blynk.virtualWrite(V3, temperature); // V3: nhietDo
      } else {
